...
```{r}
model.1.f = with(spline.data[x<5,],smooth.spline(x,y,cv=TRUE))
model.1.f
```
...
```{r}
model.1.g = with(spline.data[x>=5,],smooth.spline(x,y,cv=TRUE))
model.1.g
```
...
smoothCV <- function(x, y, K = 10, df.min = 1, df.max = 10) {
  ...
  for(df in df.min:df.max){
    ...
    for(i in 1:K){
      ...
      MSE = c(MSE,mean((predict(temp.poly,test)-test$y)^2))
      count = count+unit[i]
    }
    df.col = c(df.col,df)
    method.col = c(method.col,"poly")
    cv.error.col = c(cv.error.col,round(sum(unit/n*MSE),1))
  }
  ...
  for(df in df.min:df.max){
    if(df<3){
      df.col = c(df.col,df)
      method.col = c(method.col,"cubic.spline")
      cv.error.col = c(cv.error.col,NA)
    }else{
      MSE = c()
      count = 0
      for(i in 1:K){
        ...
        test = full[current,]
        train = full[-current,]
        temp.bs = lm(y~bs(x,df),data=train)
        MSE = c(MSE,mean((predict(temp.bs,test)-test$y)^2))
        count = count+unit[i]
      }
      df.col = c(df.col,df)
      method.col = c(method.col,"cubic.spline")
  ...
  for(df in df.min:df.max){
    if(df<=1){
      df.col = c(df.col,df)
      method.col = c(method.col,"smoothing.spline")
      cv.error.col = c(cv.error.col,NA)
    }else{
      MSE = c()
      count = 0
      for(i in 1:K){
        ...
        MSE = c(MSE,mean((predict(temp.sp,test$x)$y-test$y)^2))
        count = count+unit[i]
      }
      df.col = c(df.col,df)
      method.col = c(method.col,"smoothing.spline")
...
```{r}
model.2.d = gam(cnt~s(atemp,bs="cr",k=7)+s(mnth,bs="cr",k=6)+poly(hum,3)+s(windspeed,bs="cr",k=6),data=bikes)
...
# trial
model.test=gam(cnt~poly(hum,3)+mnth,data=bikes)
par(mfrow = c(1,2))
plot(model.test,se=TRUE,col='steelblue',lwd=3)
```
...
- **Linear model R square is 0.4889 while the additive model's R square is ..**